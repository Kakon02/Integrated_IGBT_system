...
def serial_cur_npv_nsv():
    npv = client.read_holding_registers(address=1, count=1, unit=slave_id)
    npv_result = npv.registers[0]
    if npv_result > 32767:
        npv_value = (npv_result - 65536) / 100
    else:
        npv_value = npv_result / 100
    nsv = client.read_holding_registers(address=2, count=1, unit=slave_id)
    nsv_result = nsv.registers[0]
    if nsv_result > 32767:
        nsv_value = (nsv_result - 65536) / 100
    else:
        nsv_value = nsv_result / 100
    cur_temp_main_window_text.config(state="normal")
    set_temp_main_window_text.config(state="normal")
    cur_temp_main_window_text.delete(0, tk.END)
    set_temp_main_window_text.delete(0, tk.END)
    cur_temp_main_window_text.insert(0, npv_value)
    set_temp_main_window_text.insert(0, nsv_value)
...
def serial_change_nsv():
    try:
        setting_temp_content = change_set_temp_main_window_text.get().strip()
        new_temperature = float(setting_temp_content)
        new_temperature = new_temperature * 100
        new_temperature = int(new_temperature)
        if new_temperature < 0:
            new_temperature += 65536
        setting_temp_result = client.write_register(address=21, value=new_temperature, unit=slave_id)
        if setting_temp_result.isError():
            pass
    except ValueError:
        pass
...
def serial_set_temp_p1():
    nsv = client.read_holding_registers(address=2, count=1, unit=slave_id)
    nsv_result = nsv.registers[0]
    nsv_result = int(nsv_result)
    nsv_result += 100
    if nsv_result > 65535:
        nsv_result -= 65536
    client.write_register(address=21, value=nsv_result, unit=slave_id)
...
def serial_run_loop():
    global current_path, loop_status_list, loop, timeline, operation_value
    if len(loop_status_list) == 0:
        pass
    else:
    ...
    loop = loop_status_list[:]
    value = client.read_holding_registers(address=9, count= 1, unit=slave_id)
    op_value = value.registers[0]
    if op_value == 0:
        if len(loop_status_list) == 0:
            pass
        else:
            client.write_register(address=25, value=1, unit=slave_id)
...
def check_run_loop():
    if operation_value is False:
        main_window.after_cancel(serial_run_loop_2)
        main_window.after(1000, check_run_loop)



def serial_run_loop_2():
    global loop, operation_value, loop_id
    try:
        loop_temp = loop.pop(0)
        loop_temp = float(loop_temp)
        loop_temp = 100 * loop_temp
        if loop_temp < 0:
            loop_temp += 65536
        loop_temp = int(loop_temp)
        loop_hour = loop.pop(0)
        loop_hour = int(loop_hour)
        loop_min = loop.pop(0)
        loop_min = int(loop_min)
        loop_sec = loop.pop(0)
        loop_sec = int(loop_sec)
        loop_time = (3600 * loop_hour + 60 * loop_min + loop_sec) * 1000
        client.write_register(address=21, value=loop_temp, unit=slave_id)
        if operation_value is True:
            loop_id = main_window.after(loop_time, serial_run_loop_2)
            check_run_loop()
        else:
            main_window.after_cancel(loop_id)
...
run_stop_main_window_button = tk.Button(main_window, command=run_stop_background)
